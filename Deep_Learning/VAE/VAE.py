...
class VAE:
    ...
    def compile(self, learning_rate=0.0001):
        ...
        self.model.compile(optimizer=optimizer,loss=self._calculate_loss, metrics=[self._calculate_reconstruction_loss,self._calculate_kl_loss])
    ...
    def load(self, path="."):
        ...
        autoencoder = VAE(*params)
        autoencoder._load_weigths(weights_path)
        return autoencoder
    
    def _calculate_loss(self, y_target, y_predicted):
        rec_loss = self._calculate_reconstruction_loss(y_target, y_predicted)
        kl_loss = self._calculate_kl_loss(y_target, y_predicted)
        combined_loss = self._rec_loss_weight * rec_loss + kl_loss
    ...
    def _calculate_kl_loss(self, y_target, y_predicted):
        kl_loss = -0.5 * K.sum(1+ self._log_var - K.square(self._mu) - K.exp(self._log_var), axis=1)
        return kl_loss
    ...
    def _add_enc_output(self, layers):
        '''
        Flatten data, add Gaussian Sampling, bottleneck
        '''
        self._unflat_shape = K.int_shape(layers)[1:] # shape before flat, ignore batch size
        
        x = Flatten()(layers)
        
        self._mu = Dense(self.latent_dim, name="mu")(x)
        self._log_var = Dense(self.latent_dim, name="log_var")(x)

        def p_from_norm_dist(args):
            mu, log_var = args 
            epsilon = K.random_normal(shape=K.shape(self._mu), mean=0., stddev=1.)
            point = mu + K.exp(log_var / 2) * epsilon
            return point 

        x = Lambda(p_from_norm_dist, name="encoder_output")([self._mu, self._log_var])
        return x 
